...
func (c *flagConfig) setFeatureListOptions(logger *slog.Logger) error {
	for _, f := range c.featureList {
		for o := range strings.SplitSeq(f, ",") {
			...
			case "scrape-memory-limiter":
				features.Enable(features.Scrape, "memory_limiter")
				logger.Info("Experimental scrape memory limiter enabled.")
